...
public class BoundsCheck : MonoBehaviour
{
    ...
    {
        if (!Application.isPlaying) return; //If editor not in playmode stop

        Vector3 boundSize = new Vector3(camWidth * 2, camHeight * 2, 0.1f); //Set boundary size
        Gizmos.color = Color.yellow; //Set draw color yellow
        Gizmos.DrawWireCube(Vector3.zero, boundSize); //Set wire cube to the boundary size
    } //end OnDrawGizmos()
